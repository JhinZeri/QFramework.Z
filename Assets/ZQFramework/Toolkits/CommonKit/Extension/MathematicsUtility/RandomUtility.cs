...
namespace ZQFramework.Toolkits.CommonKit.Extension.MathematicsUtility
{
    public static class RandomUtility
    {
        ...
        public static Vector2 RandomDistanceAndPos2D(float min, float max)
        {
            ...
            float symbolX = Random.Range(0, 2) == 1 ? 1f : -1f;

            // 然后随机出一个 X 的值，它的范围是最小距离到本次随机出来的长度
            // 如果 X == distance，就代表 Y 轴为0，如果 X == 0, 那么Y轴为 distance
            float randomX = symbolX * Random.Range(0, distance);

            // 确定 Y 的符号，如上
            float symbolY = Random.Range(0, 2) == 1 ? 1f : -1f;
