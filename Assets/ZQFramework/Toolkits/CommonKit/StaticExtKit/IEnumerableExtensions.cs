...
namespace ZQFramework.Toolkits.CommonKit.StaticExtKit
{
    public static class IEnumerableExtensions
    {
        ...
        {
            IEnumerable<int> poppedIndexes =
                Enumerable.Range(0, enumerable.Count()).ToList().PopRandoms(count).Select(p => p.index);
        ...
        {
            return enumerable.OrderBy(v => UnityRandom.value);
        }
        ...
        /// Throwed when <paramref name="enumerable" /> and <paramref name="probabilities" />
        /// counts are not match.
        /// </exception>
        ...
        {
            return GetRandomElementWithProbability(enumerable, (IEnumerable<float>)probabilities);
        }
        ...
        ///     <inheritdoc cref="GetRandomElementWithProbability{T}(IEnumerable{T}, float[])" path="/param[@name='enumerable']" />
        /// </param>
        /// <param name="probabilities">
        ///     <inheritdoc cref="GetRandomElementWithProbability{T}(IEnumerable{T}, float[])" path="/param[@name='probabilities']" />
        /// </param>
        /// <returns> Tuple with random element and it's index. </returns>
        /// <exception cref="ArgumentException">
        /// Throwed when <paramref name="enumerable" /> and <paramref name="probabilities" />
            ...
            IEnumerable<float> probabilities)
        {
            ...
            float randomValue = UnityRandom.value * probabilities.Sum();
            var sum = 0f;
