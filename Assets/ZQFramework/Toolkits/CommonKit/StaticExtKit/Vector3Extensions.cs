...
namespace ZQFramework.Toolkits.CommonKit.StaticExtKit
{
    public static class Vector3Extensions
    {
        ...
        static void Compare(Vector3 vector, ref int index, int compareIndex, int result)
        {
            if (vector[compareIndex].CompareTo(vector[index]) == result)
                index = compareIndex;
        }

        static int CompareAllComponents(Vector3 vector, int result)
        {
            var index = 0;
        ...
        {
            return vector[MaxComponentIndex(vector)];
        }
        ...
        {
            return vector[MinComponentIndex(vector)];
        }
        ...
        public static (Vector3 point, int index) GetClosestPoint(this Vector3 point, IEnumerable<Vector3> points)
        {
            ...
            {
                ++index;
                float distance = Vector3.Distance(point, enumerator.Current);
            ...
            Vector3 end)
        {
            ...
            float distance = Mathf.Clamp(Vector3.Dot(point - start, direction), 0f, lineMagnitude);
