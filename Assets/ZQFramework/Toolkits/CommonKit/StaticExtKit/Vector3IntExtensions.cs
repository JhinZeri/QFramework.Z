...
namespace ZQFramework.Toolkits.CommonKit.StaticExtKit
{
    public static class Vector3IntExtensions
    {
        ...
        static void Compare(Vector3Int vector, ref int index, int compareIndex, int result)
        {
            if (vector[compareIndex].CompareTo(vector[index]) == result)
                index = compareIndex;
        }

        static int CompareAllComponents(Vector3Int vector, int result)
        {
            var index = 0;
        ...
        /// <param name="points"> Compared points. </param>
        /// <returns> Closest point tuple info. </returns>
        public static (Vector3Int point, int index) GetClosestPoint(this Vector3Int point,
            IEnumerable<Vector3Int> points)
        {
            ...
            {
                ++index;
                float distance = Vector3Int.Distance(point, enumerator.Current);
        ...
        public static (Vector3 point, float distance) GetClosestPointOnRay(this Vector3Int point, Vector3 origin,
            Vector3 direction)
        {
