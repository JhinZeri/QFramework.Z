...
namespace ZFramework.Editor.Tools.TemplateScriptTool
{
    public static class ZFTemplateGen
    {
        ...
        private class ZFModelScriptAsset : EndNameEditAction
        {
            ...
            {
                // 根据模板创建脚本资产对象并获取引用。
                Object o = ZFCreateModelFromTemplate(pathName, resourceFile);
                // 在项目窗口中显示创建的资产。
                ProjectWindowUtil.ShowCreatedAsset(o);
            }


            private static Object ZFCreateModelFromTemplate(string pathName, string resourceFile)
            {
                // 获取完整的文件路径
        ...
        private class ZFSystemScriptAsset : EndNameEditAction
        {
            ...
            {
                // 根据模板创建脚本资产对象并获取引用。
                Object o = CreateScriptFromTemplateTxt.Execute(pathName, resourceFile,
